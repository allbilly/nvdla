    ...
    if (TAG_CMD == packet_id) {
        {
            ...
            oss << "MCIF SDP WR CMD addr=0x" << std::hex << payload->pd.dma_write_cmd.addr
                << " atoms=0x" << (payload->pd.dma_write_cmd.size + 1)
                << " ongoing=" << std::dec << has_sdp_onging_wr_req_
                << " got=" << sdp_wr_req_got_size_
                << " expected=" << sdp_wr_req_size_;
...
#pragma CTC ENDSKIP
        ...
        {
            std::ostringstream oss;
            oss << "MCIF SDP WR DATA ongoing=" << has_sdp_onging_wr_req_
                << " got=" << sdp_wr_req_got_size_
                << " expected=" << sdp_wr_req_size_;
            SC_REPORT_INFO(name(), oss.str().c_str());
        }
