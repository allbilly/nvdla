                    ...
                    for (channel_iter=0; channel_iter<PARALLEL_CHANNEL_NUM; channel_iter++) {
                        ...
                        if (kernel_iter==0) {
                            ...
                            if((wt_mask_k0 == 0) || (dat_mask_k0 == 0)) {
                                cslDebug((50, "CMAC_MAC_INPUT cell=%u precision=int8 mode=direct kernel=%u channel=%u active=0 dat_mask=%d wt_mask=%d data=0x%02x weight=0x%02x product=SKIP accu=0x%x\n",
                                          mac_cell_id_, kernel_iter, channel_iter, dat_mask_k0, wt_mask_k0,
                                          (uint8_t)data.to_int(), (uint8_t)weight.to_int(), (uint32_t)accu.to_int()));
                                continue;
                            }
                        }
                        else {
                            ...
                            if((wt_mask_k1 == 0) || (dat_mask_k1 == 0)) {
                                cslDebug((50, "CMAC_MAC_INPUT cell=%u precision=int8 mode=direct kernel=%u channel=%u active=0 dat_mask=%d wt_mask=%d data=0x%02x weight=0x%02x product=SKIP accu=0x%x\n",
                                          mac_cell_id_, kernel_iter, channel_iter, dat_mask_k1, wt_mask_k1,
                                          (uint8_t)data.to_int(), (uint8_t)weight.to_int(), (uint32_t)accu.to_int()));
                                continue;
                            }
                        }
                        ...
                        product = data * weight;
                        accu    = accu + product;
                        cslDebug((50, "CMAC_MAC_INPUT cell=%u precision=int8 mode=direct kernel=%u channel=%u active=1 dat_mask=1 wt_mask=1 data=0x%02x weight=0x%02x product=0x%x accu=0x%x\n",
                                  mac_cell_id_, kernel_iter, channel_iter,
                                  (uint8_t)data.to_int(), (uint8_t)weight.to_int(),
                                  (uint32_t)product.to_int(), (uint32_t)accu.to_int()));
