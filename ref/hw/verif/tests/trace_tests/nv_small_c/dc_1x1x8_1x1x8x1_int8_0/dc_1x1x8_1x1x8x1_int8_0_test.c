...
static const struct dla_reg program_initial[] = {
    ...
    { 0x3034, 0xc0000400 }, /* NVDLA_CDMA_D_DAIN_ADDR_LOW_0_0 */
...
static const uint32_t embed_0_data[][2] = {
    { 0x00000000, 0x6f000000 },
    { 0x00000004, 0x00001000 },
    { 0xFFFFFFFF, 0 }
};

static const uint32_t embed_1_data[][2] = {
    { 0x00000000, 0x00619475 },
    { 0x00000004, 0x3ffd8eaa },
    { 0xFFFFFFFF, 0 }
};
...
int main(int argc, char* argv[])
{
    ...
    { /* Zero-init region 0xc0000400 size 0x2100 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 12288, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0000000);
        if (zpage != MAP_FAILED) { memset(zpage + 1024, 0, 8448); munmap(zpage, 12288); }
    }
    { /* Zero-init region 0xd0000000 size 0x80 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xd0000000);
        if (zpage != MAP_FAILED) { memset(zpage + 0, 0, 128); munmap(zpage, 4096); }
    }
    { /* Zero-init region 0xf0000000 size 0x8 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xf0000000);
        if (zpage != MAP_FAILED) { memset(zpage + 0, 0, 8); munmap(zpage, 4096); }
    }
    ...
    { /* Data: base=0xd0000000, 2 writes */
        uint8_t* dpage = (uint8_t*)mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xd0000000);
        if (dpage != MAP_FAILED) {
    ...
    { /* CRC[0] zero-init: addr=0xf0000000 size=0x8 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xf0000000);
        if (zpage != MAP_FAILED) { memset(zpage + 0, 0, 8); munmap(zpage, 4096); }
    }
    ...
    { /* CRC[0]: addr=0xf0000000, size=0x8, expect=0x8f68a2ae */
        uint8_t* page = (uint8_t*)mmap(NULL, 4096, PROT_READ, MAP_SHARED, fd, 0xf0000000);
        if (page != MAP_FAILED) {
            uint32_t crc = calc_crc32(page + 0, 8);
            printf("CRC[0] = 0x%08x (expect 0x8f68a2ae) %s\n", crc, crc == 0x8f68a2ae ? "PASS" : "FAIL");
            if (crc != 0x8f68a2ae) failures++;
            munmap(page, 4096);
