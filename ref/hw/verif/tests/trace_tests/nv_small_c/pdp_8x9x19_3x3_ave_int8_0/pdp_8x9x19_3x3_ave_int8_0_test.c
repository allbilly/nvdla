...
static const struct dla_reg program_initial[] = {
    ...
    { 0xb040, 0x00000021 }, /* NVDLA_PDP_D_POOLING_PADDING_CFG_0 */
...
int main(int argc, char* argv[])
{
    ...
    { /* CRC[0]: addr=0xc0000000, size=0x1e6c0, expect=0x5ac59418 */
        uint8_t* page = (uint8_t*)mmap(NULL, 126976, PROT_READ, MAP_SHARED, fd, 0xc0000000);
        if (page != MAP_FAILED) {
            uint32_t crc = calc_crc32(page + 0, 124608);
            printf("CRC[0] = 0x%08x (expect 0x5ac59418) %s\n", crc, crc == 0x5ac59418 ? "PASS" : "FAIL");
            if (crc != 0x5ac59418) failures++;
            munmap(page, 126976);
