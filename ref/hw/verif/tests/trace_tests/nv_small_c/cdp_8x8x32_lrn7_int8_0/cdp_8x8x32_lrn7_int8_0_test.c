...
static const struct dla_reg program_initial[] = {
    ...
    { 0xd070, 0x00000002 }, /* NVDLA_CDP_D_LRN_CFG_0 */
    ...
    { 0xc018, 0xc0800c00 }, /* NVDLA_CDP_RDMA_D_SRC_BASE_ADDR_LOW_0 */
...
int main(int argc, char* argv[])
{
    ...
    { /* Zero-init region 0xc0800c00 size 0x2000 */
    ...
    { /* Zero-init region 0xc0000020 size 0x2000 */
    ...
    { /* Data: base=0xc0800c00, 512 writes */
        uint8_t* dpage = (uint8_t*)mmap(NULL, 12288, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0800000);
        if (dpage != MAP_FAILED) {
    ...
    { /* CRC[0] zero-init: addr=0xc0000020 size=0x2000 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 12288, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0000000);
        if (zpage != MAP_FAILED) { memset(zpage + 32, 0, 8192); munmap(zpage, 12288); }
    }
    ...
    { /* CRC[0]: addr=0xc0000020, size=0x2000, expect=0x21f0b858 */
        uint8_t* page = (uint8_t*)mmap(NULL, 12288, PROT_READ, MAP_SHARED, fd, 0xc0000000);
        if (page != MAP_FAILED) {
            uint32_t crc = calc_crc32(page + 32, 8192);
            printf("CRC[0] = 0x%08x (expect 0x21f0b858) %s\n", crc, crc == 0x21f0b858 ? "PASS" : "FAIL");
            if (crc != 0x21f0b858) failures++;
            munmap(page, 12288);
