...
static const struct dla_reg program_initial[] = {
    ...
    { 0xb070, 0xc00001e0 }, /* NVDLA_PDP_D_DST_BASE_ADDR_LOW_0 */
    { 0xb034, 0x00000707 }, /* NVDLA_PDP_D_POOLING_KERNEL_CFG_0 */
    ...
    { 0xb048, 0x000000fe }, /* NVDLA_PDP_D_POOLING_PADDING_VALUE_2_CFG_0 */
    { 0xb098, 0x00000000 }, /* NVDLA_PDP_D_PERF_WRITE_STALL_0 */
    { 0xb054, 0x0000027b }, /* NVDLA_PDP_D_POOLING_PADDING_VALUE_5_CFG_0 */
    ...
    { 0xb06c, 0x000021c0 }, /* NVDLA_PDP_D_SRC_SURFACE_STRIDE_0 */
    { 0xb050, 0x000001fc }, /* NVDLA_PDP_D_POOLING_PADDING_VALUE_4_CFG_0 */
    { 0xb040, 0x00003621 }, /* NVDLA_PDP_D_POOLING_PADDING_CFG_0 */
    ...
    { 0xb05c, 0x00000379 }, /* NVDLA_PDP_D_POOLING_PADDING_VALUE_7_CFG_0 */
    { 0xb068, 0x00000680 }, /* NVDLA_PDP_D_SRC_LINE_STRIDE_0 */
    { 0xb09c, 0x19d97a33 }, /* NVDLA_PDP_D_CYA_0 */
    { 0xb058, 0x000002fa }, /* NVDLA_PDP_D_POOLING_PADDING_VALUE_6_CFG_0 */
    ...
    { 0xa024, 0x00000680 }, /* NVDLA_PDP_RDMA_D_SRC_LINE_STRIDE_0 */
    { 0xa010, 0x00000002 }, /* NVDLA_PDP_RDMA_D_DATA_CUBE_IN_HEIGHT_0 */
...
static const uint32_t embed_0_data[][2] = {
    { 0x00000000, 0xa9f4be32 },
    { 0x00000004, 0x43b65052 },
    { 0x00000680, 0xcb629119 },
    { 0x00000684, 0xe7f62626 },
    { 0x00000d00, 0xfa2ba5f7 },
    { 0x00000d04, 0xe5840e8f },
    { 0xFFFFFFFF, 0 }
};
...
int main(int argc, char* argv[])
{
    ...
    { /* Zero-init region 0xc0002c00 size 0x21c0 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 12288, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0002000);
        if (zpage != MAP_FAILED) { memset(zpage + 3072, 0, 8640); munmap(zpage, 12288); }
    }
    { /* Zero-init region 0xc00001e0 size 0x26c0 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 12288, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0000000);
        if (zpage != MAP_FAILED) { memset(zpage + 480, 0, 9920); munmap(zpage, 12288); }
    }

    /* Load embedded data (replaces devmem load scripts) */
    { /* Data: base=0xc0002c00, 6 writes */
        uint8_t* dpage = (uint8_t*)mmap(NULL, 8192, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0002000);
        if (dpage != MAP_FAILED) {
    ...
    { /* CRC[0] zero-init: addr=0xc00001e0 size=0x26c0 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 12288, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0000000);
        if (zpage != MAP_FAILED) { memset(zpage + 480, 0, 9920); munmap(zpage, 12288); }
    }
    ...
    { /* CRC[0]: addr=0xc00001e0, size=0x26c0, expect=0xad5a0fc3 */
        uint8_t* page = (uint8_t*)mmap(NULL, 12288, PROT_READ, MAP_SHARED, fd, 0xc0000000);
        if (page != MAP_FAILED) {
            uint32_t crc = calc_crc32(page + 480, 9920);
            printf("CRC[0] = 0x%08x (expect 0xad5a0fc3) %s\n", crc, crc == 0xad5a0fc3 ? "PASS" : "FAIL");
            if (crc != 0xad5a0fc3) failures++;
            munmap(page, 12288);
