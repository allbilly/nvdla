...
static const struct dla_reg program_initial[] = {
    ...
    { 0x9058, 0x00000004 }, /* NVDLA_SDP_D_DP_BS_CFG_0 */
    ...
    { 0x9078, 0x00000200 }, /* NVDLA_SDP_D_DP_BN_MUL_CFG_0 */
    ...
    { 0x906c, 0x00000000 }, /* NVDLA_SDP_D_DP_BN_CFG_0 */
...
int main(int argc, char* argv[])
{
    ...
    { /* Zero-init region 0xc0000100 size 0xa00 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0000000);
        if (zpage != MAP_FAILED) { memset(zpage + 256, 0, 2560); munmap(zpage, 4096); }
    }
    ...
    { /* CRC[0]: addr=0xd00001c0, size=0xf00, expect=0xdb8d3a94 */
        uint8_t* page = (uint8_t*)mmap(NULL, 8192, PROT_READ, MAP_SHARED, fd, 0xd0000000);
        if (page != MAP_FAILED) {
            uint32_t crc = calc_crc32(page + 448, 3840);
            printf("CRC[0] = 0x%08x (expect 0xdb8d3a94) %s\n", crc, crc == 0xdb8d3a94 ? "PASS" : "FAIL");
            if (crc != 0xdb8d3a94) failures++;
            munmap(page, 8192);
