...
static const struct dla_reg program_initial[] = {
    ...
    { 0x9014, 0x000100c1 }, /* NVDLA_SDP_S_LUT_INFO_0 */
    { 0x9020, 0x53b19eb3 }, /* NVDLA_SDP_S_LUT_LO_START_0 */
    { 0x9024, 0x53b1a0b3 }, /* NVDLA_SDP_S_LUT_LO_END_0 */
    { 0x9028, 0x12b36409 }, /* NVDLA_SDP_S_LUT_LE_SLOPE_SCALE_0 */
    { 0x902c, 0x000002dc }, /* NVDLA_SDP_S_LUT_LE_SLOPE_SHIFT_0 */
    { 0x9030, 0x280c249c }, /* NVDLA_SDP_S_LUT_LO_SLOPE_SCALE_0 */
    { 0x9034, 0x00000205 }, /* NVDLA_SDP_S_LUT_LO_SLOPE_SHIFT_0 */
...
int main(int argc, char* argv[])
{
    ...
    { /* CRC[0]: addr=0xd00001c0, size=0xf00, expect=0x3347874c */
        uint8_t* page = (uint8_t*)mmap(NULL, 8192, PROT_READ, MAP_SHARED, fd, 0xd0000000);
        if (page != MAP_FAILED) {
            uint32_t crc = calc_crc32(page + 448, 3840);
            printf("CRC[0] = 0x%08x (expect 0x3347874c) %s\n", crc, crc == 0x3347874c ? "PASS" : "FAIL");
            if (crc != 0x3347874c) failures++;
            munmap(page, 8192);
