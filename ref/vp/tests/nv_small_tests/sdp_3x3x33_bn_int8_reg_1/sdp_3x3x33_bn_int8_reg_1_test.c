...
static const struct dla_reg program_initial[] = {
    ...
    { 0x906c, 0x00000050 }, /* NVDLA_SDP_D_DP_BN_CFG_0 */
...
int main(int argc, char* argv[])
{
    ...
    { /* CRC[0]: addr=0xd00001c0, size=0xf00, expect=0x51a9869b */
        uint8_t* page = (uint8_t*)mmap(NULL, 8192, PROT_READ, MAP_SHARED, fd, 0xd0000000);
        if (page != MAP_FAILED) {
            uint32_t crc = calc_crc32(page + 448, 3840);
            printf("CRC[0] = 0x%08x (expect 0x51a9869b) %s\n", crc, crc == 0x51a9869b ? "PASS" : "FAIL");
            if (crc != 0x51a9869b) failures++;
            munmap(page, 8192);
