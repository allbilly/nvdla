...
static const struct dla_reg program_initial[] = {
    ...
    { 0xd050, 0xc0800020 }, /* NVDLA_CDP_D_DST_BASE_ADDR_LOW_0 */
    ...
    { 0xc014, 0x0000001e }, /* NVDLA_CDP_RDMA_D_DATA_CUBE_CHANNEL_0 */
...
static const uint32_t embed_0_data[][2] = {
    { 0x00000000, 0x913217a9 },
    { 0x00000004, 0x99e550e7 },
    { 0x00000800, 0x82a5b6f6 },
    { 0x00000804, 0x62fa1985 },
    { 0x00001000, 0x52c62b7f },
    { 0x00001004, 0x84f70e8f },
    { 0x00001800, 0xbef42643 },
    { 0x00001804, 0x0026cbb5 },
    { 0xFFFFFFFF, 0 }
};
...
int main(int argc, char* argv[])
{
    ...
    { /* Zero-init region 0xc0800020 size 0x2000 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 12288, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0800000);
        if (zpage != MAP_FAILED) { memset(zpage + 32, 0, 8192); munmap(zpage, 12288); }
    }

    /* Load embedded data (replaces devmem load scripts) */
    { /* Data: base=0xc0000c00, 8 writes */
    ...
    { /* CRC[0] zero-init: addr=0xc0800020 size=0x2000 */
        uint8_t* zpage = (uint8_t*)mmap(NULL, 12288, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0xc0800000);
        if (zpage != MAP_FAILED) { memset(zpage + 32, 0, 8192); munmap(zpage, 12288); }
    }
    ...
    { /* CRC[0]: addr=0xc0800020, size=0x2000, expect=0x7a762723 */
        uint8_t* page = (uint8_t*)mmap(NULL, 12288, PROT_READ, MAP_SHARED, fd, 0xc0800000);
        if (page != MAP_FAILED) {
            uint32_t crc = calc_crc32(page + 32, 8192);
            printf("CRC[0] = 0x%08x (expect 0x7a762723) %s\n", crc, crc == 0x7a762723 ? "PASS" : "FAIL");
            if (crc != 0x7a762723) failures++;
            munmap(page, 12288);
